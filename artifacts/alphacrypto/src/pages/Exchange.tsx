...
const Exchange = () => {
  ...
  const submit = async () => {
    if (!validPair) { toast.error("Faqat crypto ↔ UZS"); return; }
    if (!fromAmount || !toAmount) { toast.error("Summalarni kiriting"); return; }
    if (minHint) {
      const check = minHint.side === "from" ? Number(fromAmount) : Number(toAmount);
      if (check < minHint.amount) { toast.error(`Minimal: ${minHint.amount} ${minHint.sym}`); return; }
    }
    if (!receiveAddr.trim()) { toast.error("Qabul qilish manzilini kiriting"); return; }
    if (!file) { toast.error("Toʻlov screenshotini yuklang"); return; }
    if (!sendToWallet) { toast.error(`${from} uchun manzil yoʻq`); return; }
